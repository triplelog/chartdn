...
exports.pivotHeaders = function(options,hArray,types){
	var hArrayNew = [];
	if (hArray[options.pivot]){
		hArrayNew.push(hArray[options.pivot]);
	}
	for (var ii in options.columns) {
		hArrayNew.push(hArray[options.columns[ii].column] + '(' + options.columns[ii].type + ')');
	}
	
	idx = 0;
	iidx = 0;
	for (var ii in hArrayNew){
		hArray[iidx]=hArrayNew[ii];
		iidx++;
	}
