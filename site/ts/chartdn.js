...
ws.onopen = function (evt) {
    ...
    if (dataid != "") {
        jsonmessage['dataid'] = dataid;
    }
    if (chartidtemp != "") {
        jsonmessage['chartidtemp'] = chartidtemp;
    }
...
//Update Headers
function updateHeaders(initialData, chg) {
    if (chg === void 0) { chg = false; }
...
function optionsChg(optionname) {
    if (optionname == 'xaxistitle' || optionname == 'yaxistitle') {
        ...
        if (optionname == 'xaxistitle') {
            jsonmessage['labels']['x'] = newoption;
        }
        else if (optionname == 'yaxistitle') {
            jsonmessage['labels']['y'] = newoption;
        }
        ws.send(JSON.stringify(jsonmessage));
    }
        ...
        if (optionname == 'stepSizeX') {
            jsonmessage['stepSize']['x'] = newoption;
        }
        else if (optionname == 'stepSizeY') {
            jsonmessage['stepSize']['y'] = newoption;
        }
        ws.send(JSON.stringify(jsonmessage));
    }
...
function updateModifiedTable(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (i < nHeaders) {
            ...
            newcell.textContent = i - nHeaders;
        }
...
function dataChg(initialData, dataType) {
    if (initialData === void 0) { initialData = false; }
    if (dataType === void 0) { dataType = 'csv'; }
...
var drake = dragula([document.getElementById('yAxisDataBox')], {
    ...
    accepts: function (el, target, source) {
        if (target === source || target.id === 'xColumn') {
            return true;
        }
        for (var yid in yColsVals) {
            if ('colId' + yColsVals[yid] == el.id) {
                return false;
            }
        }
        return true;
                ...
                if (el.getAttribute('name') == 'formula') {
                    ...
                    if (type == 'value') {
                        ...
                        if (rowtype == 'previous') {
                            ...
                            row = ell.querySelector('input[name=aftern]').value.toString();
                        }
                        else if (rowtype == 'equal') {
                            row = '$' + parseInt(ell.querySelector('input[name=equalrow]').value).toString();
                        }
                    }
                    else {
                        var rowtype = ell.querySelector('input[name=rowstart' + modifiers[i].id + ']:checked').value;
                        if (rowtype == 'previous') {
                            row = '-' + parseInt(ell.querySelector('input[name=prevnstart]').value).toString();
                        }
                        else if (rowtype == 'next') {
                            row = ell.querySelector('input[name=afternstart]').value.toString();
                        }
                        else if (rowtype == 'equal') {
                            row = '$' + parseInt(ell.querySelector('input[name=equalrowstart]').value).toString();
                        }
                        rowtype = ell.querySelector('input[name=rowend' + modifiers[i].id + ']:checked').value;
                        if (rowtype == 'previous') {
                            row += ',-' + parseInt(ell.querySelector('input[name=prevnend]').value).toString();
                        }
                        else if (rowtype == 'next') {
                            row += ',' + ell.querySelector('input[name=afternend]').value.toString();
                        }
                        else if (rowtype == 'equal') {
                            row += ',$' + parseInt(ell.querySelector('input[name=equalrowend]').value).toString();
                        }
                    }
...
function chgModify(mObject) {
    ...
    for (var i in modifiers) {
        ...
        if (!document.getElementById('edit' + m.id)) {
            continue;
        }
        if (m.id == mObject.id) {
...
function toRowStr(objVar) {
    ...
    if (objVar.type == 'value' && objVar.row != '0') {
        if (objVar.row.indexOf('$') == 0) {
            rowStr += ', Row ' + objVar.row.substring(1);
        }
        else if (parseInt(objVar.row) < 0) {
            rowStr += ', ' + parseInt(objVar.row) * -1 + ' Row Before';
        }
        else {
            rowStr += ', ' + parseInt(objVar.row) + ' Row After';
        }
    }
    if (objVar.type != 'value' && objVar.row != '0,-1') {
        var rows = objVar.row.split(',');
        if (rows[0].indexOf('$') == 0) {
            rowStr += ', Row ' + rows[0].substring(1);
        }
        else if (parseInt(rows[0]) < 0) {
            rowStr += ', ' + parseInt(rows[0]) * -1 + ' Row Before';
        }
        else if (parseInt(rows[0]) == 0) {
            rowStr += ', Current Row';
        }
        else {
            rowStr += ', ' + parseInt(rows[0]) + ' Row After';
        }
        if (rows[1].indexOf('$') == 0) {
            rowStr += ' to Row ' + rows[1].substring(1);
        }
        else if (parseInt(rows[1]) < 0) {
            rowStr += ' to ' + parseInt(rows[1]) * -1 + ' Row Before';
        }
        else if (parseInt(rows[1]) == 0) {
            rowStr += ' to Current Row';
        }
        else {
            rowStr += ' to ' + parseInt(rows[1]) + ' Row After';
        }
    }
    return rowStr;
}
...
function createNewModifier(show) {
...
var drakeF = dragula([document.getElementById('allModifiers')], {
    ...
    accepts: function (el, target, source) {
        if (target === document.getElementById('allModifiers')) {
            return true;
        }
        else {
            return false;
...
function minimizeBox(boxid, full) {
...
function showMoreOptions(collapse) {
